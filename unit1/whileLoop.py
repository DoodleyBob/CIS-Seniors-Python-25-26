...
theSum = 0.0
data = input("Enter a number for just enter to quit: ")
while data != "":
    number = float(data)
    theSum += number
    data = input("Etner a number for just enter to quit: ")
...
while count <= 10000:
    theSum += count
    count += 1  # Control Variable needs to increase
print(theSum)


print("\n\nCounting down with loops")

# Countin down with a for loop
...
while not done:     # while not == False
    ...
    if number >= 0 and number <= 100:
        ...
        print("Error: grade mmust be between 100 and 0.")
